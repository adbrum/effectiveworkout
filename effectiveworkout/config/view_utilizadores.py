...
from effectiveworkout.config.form_utilizadores import AddUserForm, AddGroupForm, \
    EditUserForm, PasswordChangeFormReset, EditGroupForm, MeuPerfilForm, \
    FichaUserForm, FichaGroupForm, FichaMeuPerfilForm
from effectiveworkout.usersprofiles.forms import EditUserProfileForm
...
def addUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            warning = True
            warningMensage = ('Atenção existem campos por preencher!')
            listaErros.append(warningMensage)
...
def confirmDesativarUsers(request, *args, **kwargs):
    ...
    if request.is_ajax():
        try:
            ...
            message = ('Deve selecionar pelo menos um utilizador!')
    ...
    if request.is_ajax():
        ...
        if listaIdsGrupos != []:
            ...
            for id_perfil in listaIdsGrupos:
                ...
                isApagar = True
                PERGUNTA_0 = ('Do(s) Grupo(os) selecionados pode apagar os seguintes:')
                strUrl += str(id_perfil) + "_"

        else:
            TITULO_MODAL = ('Alerta')
            ALERTA = ('Deve selecionar o grupo que deseja apagar!')

        PERGUNTA = ('Deseja mesmo apagar o(s) grupo(s)?')
...
def meuPerfil(request, *args, **kwargs):
    ...
    utilizador = User.objects.get(pk=idUser)
    ...
    if request.method == 'POST':
        form = MeuPerfilForm(argUser, argEmail, userAtual, request.POST or None, instance=utilizador)  # An unbound form
        form2 = EditUserProfileForm(request.POST or None)

        if (form.is_valid() and form2.is_valid()):
            # form.save()
            # form2.save()
            User.objects.filter(pk=idUser).update(username=form.cleaned_data['username'],
                                                  first_name=form.cleaned_data['first_name'],
                                                  last_name=form.cleaned_data['last_name'],
                                                  email=form.cleaned_data['email'],
                                                  )

            perfil = UserProfile.objects.filter(user_id=idUser).update(**form2.cleaned_data)
            # person, created = UserProfile.objects.get_or_create(user_id=idUser)
            # if created:
            #     # A new person object created
            #     UserProfile.objects.create(**form2.cleaned_data)
            #     print('CRIAR')
            # else:
            #     # person object already exists
            #     UserProfile.objects.update(**form2.cleaned_data)
            #     print('EDITAR')
        ...
        print('GEEEEEEEEEEEEEEEEEEEE')

        perfil = UserProfile.objects.get(user_id=idUser)
        form = MeuPerfilForm(argUser, argEmail, userAtual, instance=utilizador)  # An unbound form
        form2 = EditUserProfileForm(instance=perfil)
