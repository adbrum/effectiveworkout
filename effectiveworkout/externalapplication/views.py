...
from effectiveworkout.usersprofiles.forms import UserProfileForm, AddUserForm #, HealthAnamneseForm
...
def empty_form(request):
    return render(request, 'externalapplication/add.html',
                  {'form': AddUserForm(), 'form_02': UserProfileForm()})


# def empty_form(request):
#     return render(request, 'externalapplication/add.html',
#                   {'form': UserProfileForm(), 'form_02': HealthAnamneseForm()})
...
def create(request):
    ...
    if request.method == 'POST':
        ...
        form_02 = UserProfileForm(request.POST or None)

        if (form.is_valid() and form_02.is_valid()):
            ...
            perfil = UserProfile.objects.create(**form_02.cleaned_data, user_id=user.pk)
    ...
    #     atleta = UserProfile.objects.create(**form.cleaned_data)
    #
